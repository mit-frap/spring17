 ...
 * that they are equivalent when they pass the checker.  Now we can use an
 * unproven function to make the transformation, so long as it always happens to
 * transform programs into compatible counterparts.  A bit more formally, we
...
Fixpoint cmdNotReading (c: cmd) (md: maydiff) :=
  ...
  | Skip => true
  | Nop => true
  | Assign _ e => arithNotReading e md
 ...
 *
 * We use [Nop] instead of [Skip] keep a 1-1 execution correspondence between
 ...
 *
 * [validator'] returns a pair, saying (nmd, result), where "nmd" is a 
 * post-maydiff set after validating a single command, and "result" tells if
 * it succeeds to validate the given programs. *)
Fixpoint validator' (s t: cmd) (md: maydiff) :=
  match s, t with
  | Nop, Nop => (md, true)
  | Skip, Skip => (md, true)
  | Assign sx se, Assign tx te =>
    (md, if md $? tx then false
                     (* ^-- Give up if the assigned variable is already
                      *     different between the two sides. *)
         else if cmd_eq_dec s t
              then arithNotReading se md
                   (* ^-- Since the commands are the same, we succeed if the
                    *     expression being assigned only uses variables that the
                    *     two sides agree on. *)
              else false)
  | Nop, Assign x (Const n) => (md $+ (x, n), if md $? x then false else true)
                               (* ^-- A new variable to disagree on!  The right
                                *     side has the assignment, but the left
                                *     doesn't. *)
  | Assign x (Const n), Nop =>
     ...
     * correctness proof difficult, so it is for adjusting the difficulty! *)
    (md, match md $? x with
  ...
  | If se sc1 sc2, If te tc1 tc2 =>
     ...
     * motion. *)
    let (_, b1) := validator' sc1 tc1 md in
    let (_, b2) := validator' sc2 tc2 md in
    (md, if cmd_eq_dec s t then b1 && b2 && cmdNotReading s md else false)
  | While se sc, While te tc =>
    let (_, b1) := validator' sc tc md in
    (md, if cmd_eq_dec s t then b1 && arithNotReading se md else false)
  | Output se, Output te => (md, if arith_eq_dec se te
                                 then arithNotReading se md
                                 else false)
  | _, _ => (md, false)
  end.

Definition validator (s t: cmd) := snd (validator' s t $0).
